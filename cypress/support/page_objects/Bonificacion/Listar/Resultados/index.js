...
class BonificacionListarResultados {


    verificarResultadosBonificaciones() {
        // Recupera los valores guardados con alias y los usa
        cy.get('@id').then((id) => {
            cy.get('@tipo').then((tipo) => {
                cy.get('@semestre').then((semestre) => {
                    // Verifica que la cabecera muestra los valores esperados
                    cy.get(el.cabecera)
                        .should('be.visible')
                        .should('contain', 'Resultados de la Bonificación')
                        .should('contain', id.trim())
                        .should('contain', tipo.trim())
                        .should('contain', semestre.trim());
...
export default new BonificacionListarResultados();